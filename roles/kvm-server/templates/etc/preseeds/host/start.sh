...
virt-install --name {{ hostvars[item]['preseed_hostname'] }} \
  ...
  --vcpus {{ hostvars[item]['preseed_vcpus'] }} --os-type linux --os-variant ubuntu18.04 \
  --network bridge=br0,mac={{ hostvars[item]['preseed_macaddr'] }} \
  --graphics vnc,listen=127.0.0.1 \
  --noautoconsole --location 'http://archive.ubuntu.com/ubuntu/dists/bionic/main/installer-amd64/' \
  --autostart \
